 ...
 */
function validateExtractionTemplate(template: string | undefined): string | undefined {
 ...
 */
function sanitizeRawResponse(raw: Record<string, unknown> | undefined): Record<string, unknown> | null {
  if (!raw) return null;
...
export async function uploadInvoice(request: Request, ctx: RequestContext): Promise<Response> {
  ...
  if (
    ...
    const { data: supplierFull } = await ctx.serviceClient
      .from('suppliers')
      .select('id, name, code, cor_vendor_code, cor_customer_code, cor_community_code, cor_transaction_type, cor_currency_code, cor_field_mapping, cor_mapping_config, cor_ingestion_enabled')
      .eq('id', supplierId)
      .single();
