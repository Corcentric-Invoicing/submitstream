...
export async function submitToCorcentricHandler(
  ...
  // Resolve community code: prefer community_id FK → communities.code, fallback to cor_community_code text
  ...
  if (!supplier.cor_vendor_code || !supplier.cor_community_code) {
    return errorResponse(
      'Supplier is missing Corcentric codes (corVendorCode or corCommunityCode). Configure these in supplier settings.',
      400,
    );
  }
  ...
  // 3. Final fallback: supplier default
