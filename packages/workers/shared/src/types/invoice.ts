...
/** @deprecated Use IngestionSource. Retained for back-compat during migration. */
export type InvoiceSource = 'email' | 'upload';
/**
 * How an invoice entered the system. Extensible — new connectors
 * (QuickBooks, NetSuite, etc.) add entries here AND in the
 * invoices_ingestion_source_check constraint in Postgres.
 */
export type IngestionSource = 'email' | 'upload' | 'promostandards';
...
/**
 * Structured result from the invoice validator. Persisted in
 * invoices.validation_findings so the portal can render flag chips
 * without re-running rules on every render.
 */
export interface ValidationFinding {
  severity: ValidationSeverity;
  /** Stable machine-readable identifier, e.g. 'TAX_ARRAY_SUM_MISMATCH'. */
  code: string;
  /** Human-readable explanation surfaced to reviewers. */
  message: string;
  /** Dotted path into the invoice where the issue lives (optional). */
  field?: string;
}
...
export interface Invoice {
  ...
  /** @deprecated Use `ingestion_source`. Kept populated for back-compat. */
