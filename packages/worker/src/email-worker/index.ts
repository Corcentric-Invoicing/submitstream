...
import { validateFlatInvoice } from '../api/validation/invoice-validator';
  ...
  async email(message: ForwardableEmailMessage, env: EmailWorkerEnv): Promise<void> {
    ...
    for (const attachment of pdfAttachments) {
      ...
      try {
        ...
        // Run the Corcentric DMS validator over the flat invoice_data
        // shape. Findings get persisted so the queue + review UI can show
        // warning chips and block submission on error-severity. If the
        // validator finds errors we force status='pending' regardless of
        // OCR confidence.
        const validationFindings = validateFlatInvoice(enrichedData as Record<string, unknown>);
        ...
        const updateData: Record<string, unknown> = {
          // Validator-flagged errors override OCR's status — block submission until review
          status: hasValidationErrors ? 'pending' : ocrResult.status,
          ...
          needs_supplier_review: ocrResult.status === 'rejected' || hasValidationErrors,
