...
export async function previewCorcentricXml(request: Request, ctx: RequestContext): Promise<Response> {
  ...
  // ── Pick primary supplier_communities row (post-refactor) ──
  // Per-community vendor/customer codes live on the join. Legacy
  // supplier.cor_* columns are still selected as a transition-period fallback.
  ...
  // 3. Fall back to per-community default, then legacy supplier-level
